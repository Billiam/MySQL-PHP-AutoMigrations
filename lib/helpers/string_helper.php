...
class MpmStringHelper
{
	...
	*/
	...
	{
		//Combination match and extract
		if(!preg_match('|^(\d{4})_(\d{2})_(\d{2})_(\d{2})_(\d{2})_(\d{2}).*\.php5?$|',$file,$t))
			return null;

		$timestamp = $t[1] . '-' . $t[2] . '-' . $t[3] . 'T' . $t[4] . ':' . $t[5] . ':' . $t[6];

		// validate the date
		if (false === checkdate($t[2], $t[3], $t[1]))
		{
			return null;
		}
		// validate timestamp
	...
	*/
	...
	{
		return date('Y_m_d_H_i_s', strtotime($timestamp));
	}
